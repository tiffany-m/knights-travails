// TOP's directions were super vague, referenced this video https://www.youtube.com/watch?v=vSjyklN-GEs&list=PLL_vPD7G8X6v3mcnPkc2PpPjC-O5z6YW_&index=14
// video suggests doing this exercise - https://www.khanacademy.org/computing/computer-science/algorithms/breadth-first-search/pc/challenge-implement-breadth-first-search
...
function containsSpot(arr, target) {
  if (
    arr.find((element) => element[0] === target[0]) &&
...
function buildInfoArr(boardArr, startIndex) {
  ...
  for (let i = 0; i < boardArr.length; i++) {
    newArr[i] = {
      distance: null, // vertex before current one
      predecessor: null, // shortest path (in terms of the number of edges) from the source
    };
  }
  ...
  while (u != endIndex) {
    ...
    for (let i = 0; i < adjList[u].length; i++) {
      ...
      if (vIndex === endIndex) {
        ...
        console.log(
          `You made it in ${path.length - 1} moves! Here's your path: ${path}`
        );
        return path;
      } else {
